...
Read the `xlsx` skill, then build per `references/spreadsheet-style.md`, which uses the **bundled `scripts/trip_xlsx.py` builder** — import it rather than re-writing styling/hyperlink/audit/recalc boilerplate each run (that rewrite is slow and is where link bugs creep in). Your build code holds only the trip-specific tabs, columns, and cost formulas. From the gathered options, select a recommended option per segment (price + reviews + location, within budget), confirming with the user. Tabs: **Trip Summary** (always), **Hotels** (always, names hyperlinked), **Flights** (always, nonstop preferred), **Activities** (priced, with links), **Itinerary** (when useful). Inputs blue, formulas black, cross-sheet links green, hyperlinks blue-underlined; currency format throughout.

**Per-person figures.** Every cost surface shows a **per-person** value alongside the group total, and per-person is the emphasized column (it answers "what does one flight / one room-share cost me"). In the Trip Summary cost breakdown the per-person column sits *before* the group total. Build with `tx.per_person(total_cell, travelers_cell)`.

**Switchable picks.** Hotels and Flights each get a **Choose** column (dropdown via `tx.choose_validation`, recommended row pre-marked with `tx.choose_mark` + CHOSEN fill). The Trip Summary pulls each segment's chosen total with a SUMIF over that segment's rows (`tx.chosen_total(total_range, marker_range, sheet=…)`) rather than a hardcoded cell — so the user can pick a different hotel or flight and the trip total updates on recalc, no formula editing.
...
**Link sanity check (live) — the last gate before presenting.** The coverage gate above only proves each priced row *has* a link; this step proves each link actually *works*. After `tx.finalize()` passes, run `tx.check_links(path)` — it fetches every hyperlink in the workbook and sorts the results into **ok / blocked / broken**. A `blocked` result means a big travel site (Booking.com, Expedia, Tripadvisor, lastminute.com, …) refused the automated request with a 403/429/timeout — there is nothing to fix, the link is fine for the human who clicks it, so accept it. A `broken` result is a real problem: a dead domain, a 404/410, a 5xx, or a page that returns 200 but whose **content** reads as gone ("page not found", "no longer available", "no results found"). **For every entry in `report["broken"]`, spin up a focused fix-task** (a subagent when available, otherwise an inline sub-step) that (1) opens the URL to see the actual failure, (2) works out *why* it broke — an expired/!wrong deep link, bad query params, a moved page, or a bug in the URL builder — (3) recovers a working replacement (the correct deep link, or a search URL for that exact hotel/flight/activity on the same provider), (4) patches the cell with `tx.link(...)`, and (5) re-runs `tx.check_links` on the saved file. Loop until `report["broken"]` is empty. Record in each fix-task what was wrong and what replaced it, so a recurring builder bug (e.g. a malformed Expedia URL template) gets fixed at the source rather than one link at a time. Only present once the broken list is clear.

Save to `/mnt/user-data/outputs/`, `present_files`, and give a short summary (total, per person, chosen picks) — and confirm both the link-coverage audit and the live link sanity check passed (noting any links that were bot-blocked and accepted).
